...
parser.add_argument('--NP-path', type=str, default='./resnet',
...
parser.add_argument('--E', type=float, default=0.3,
                    help='max distoration')
parser.add_argument('--LR', type=float, default=0.01,
                    help='max distoration')
parser.add_argument('--images-num', type=int, default=100,
                    help='Test Image')
parser.add_argument('--attack-path', type=str, default='./resnet',
...
def NP_Attack_Targeted(model, x_input, y_label, target, N, attack_model):
    """NP(ANP) estimator"""
    ...
    for iter in range(nb_iter):
        ...
        model_pred = y_pred.item()
        if iter%10==0:
            print('Prediction: ', y_pred.item(), 'Label:', y_label.item(), 'Target', target, 'Confidence',
                  pred_logit[0, target].item())
        if y_pred.item() == target:
            print ('Attack Success: Predict: %d Real: %d Target: %d'% (y_pred.item(), y_label.item(), target))
            if cnt < log_time:
        ...
        pred_logit = F.softmax(attack_model(y_pred), dim=1).cpu().detach()
        # loss, confidence = PGD_loss(pred_logit, y_label)
        ...
        if iter%10 == 0:
            print ('Iter %d Adv LOSS %.4f Model Pred %d' % (iter, pgd_loss, model_pred))
            if pgd_loss > last_loss and lr > 1e-3:
                lr = lr/2
...
for batch_idx, (y_all, y_label) in enumerate(test_loader):
    if num_images > args.images_num:
        break
    pred = attack_model(y_all.cuda()).data.cpu().numpy()
    y_pred = np.argmax(pred,axis=1)
    if y_pred.item() != y_label.item():
        continue
    batch_size = y_all.shape[0]
    if args.dataset == 'mnist' or args.dataset == 'adv_mnist':
    ...
    for t in range(10):
        if t == y_label.item():
            ...
            Test_Image += 1
            perturbation, attack, iters, pred = NP_Attack_Targeted(NP_model, y_all, y_label, t, args.N, attack_model)
            if attack == True:
                attack_success += 1
                avg_iters += iters
                ...
                print (log_msg)
                if os.path.exists('./%s'%(root_path)) == False:
                    os.mkdir('./%s'%(root_path))
                if attack_success == 1:
                    with open('./%s/log_%s_%.3f_%d_%.3f_%d_%s_%s_targeted.txt' % (root_path, args.dataset, args.E, args.N, args.LR, args.I, args.model, args.type), 'w') as f:
                        f.write(log_msg)
                else:
                    with open('./%s/log_%s_%.3f_%d_%.3f_%d_%s_%s_targeted.txt' % (root_path, args.dataset, args.E, args.N, args.LR, args.I, args.model, args.type), 'a') as f:
                        f.write(log_msg)

    if len(attack_success_images) > 4 and cnt < log_time:
        AS_IMAGE = torch.cat(attack_success_images, dim=0)
        ORI_IMAGE = torch.cat(test_images, dim=0)
        Exp_Name = '%s_%d_%d_%.2f_%s_targeted' % (args.dataset, args.I, args.N, args.E, args.type)
        if args.dataset == 'mnist':
...
log_msg = 'Attacking %d Images Attack Success Rate: %.4f Avg Iter: %.4f Avg Query: %d Avg L1 %.4f Avg L2 %.4f Avg Lp %.4f \n ' % \
                  (Test_Image, ASR, AI, AI * args.N, avg_L1 / attack_success, avg_L2 / attack_success,
                   avg_Lp / attack_success)
with open('%s/log_%s_%.3f_%d_%d_%d_%s_%s_targeted.txt' % (root_path, args.dataset, args.E, args.N, args.LR, args.I, args.model, args.type), 'a') as f:
    f.write(log_msg)
