...
class NP(nn.Module):
    def __init__(self, args):
        ...
        self.h_7 = nn.Linear(2, self.hidden_dim)
        self.h_8 = nn.Linear(self.hidden_dim, 2)

        self.h_9 = nn.Linear(2, self.hidden_dim)
        self.h_10 = nn.Linear(self.hidden_dim, 2)
        ...
        for i, (y_all, _) in enumerate(test_loader):
            ...
            if i == 0:  # save PNG of reconstructed examples
                ...
                for N in plot_Ns:
                    ...
                    recons = torch.cat(recons).permute(0, 2, 1).view(-1, c, channel_dim, channel_dim).expand(-1, 3, -1,
                                                                                                             -1)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--restore_model', type=str, default='np_pretrain/mnist_128_test',
    ...
    parser.add_argument('--log_dir', type=str, default='./log/mnist_np_multihead_64',
                        help='path to log')
    ...
    if args.dataset == 'mnist':
        ...
        transform_ = transforms.Compose([RGB2Y(), transforms.ToTensor(), transforms.Normalize(mean=[0.5, 0.5, 0.5],
                                                                                              std=[1, 1, 1])])
