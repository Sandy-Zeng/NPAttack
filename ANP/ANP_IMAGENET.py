...
parser.add_argument('--data-path', type=str, default='/data/dataset/ILSVRC2012',
                    help='path to data')
...
parser.add_argument('--restore_model', type=str, default='np_pretrain/imagenet_128',
...
parser.add_argument('--log_dir', type=str, default='./log/imagenet_np_multihead_128',
                    help='path to log')
...
if args.dataset == 'imagenet':
    ...
    transformers = transforms.Compose([
        transforms.Resize((channel_dim, channel_dim)),
        # transforms.CenterCrop(224),
        transforms.ToTensor(),
        normalize,
    ])
    train_dataset = datasets.ImageFolder(traindir, transformers)
...
class NP(nn.Module):
    def __init__(self, args):
        ...
        self.q_conv = nn.Conv1d(in_channels=args.r_dim, out_channels= args.r_dim // 8, kernel_size=1)
        self.k_conv = nn.Conv1d(in_channels=args.r_dim, out_channels= args.r_dim // 8, kernel_size=1)
        self.v_conv = nn.Conv1d(in_channels=args.r_dim, out_channels=args.r_dim, kernel_size=1)
    ...
    def self_attention(self, input_xy):
        ...
        q = self.q_conv(input_xy)
        # print('q-------------------',q.shape)
        q = q.view(m_batchsize, -1, width_height).permute(0, 2, 1)
        k = self.k_conv(input_xy).view(m_batchsize, -1, width_height)
        energy = torch.bmm(q, k)
        attention = nn.functional.softmax(energy, dim=-1)
        v = self.v_conv(input_xy).view(m_batchsize, -1, width_height)
    ...
    def forward(self, x_context, y_context, x_all=None, y_all=None):
        ...
        y_mean, sigma = self.decoder(r_sample, z_sample, x_target)

        return y_mean, sigma, z_all[0], z_all[1], z_context[0], z_context[1]
...
def np_loss(y_mean,sigma, y, z_all, z_context):
...
def train(epoch):
    ...
    for batch_idx, (y_all, _) in enumerate(train_loader):
        ...
        optimizer.zero_grad()
        y_mean, sigma, z_all_mu, z_all_var, z_con_mu, z_con_var = model(x_context, y_context, x_all, y_all)
        # print(y_hat[0].shape)
        z_all = (z_all_mu, z_all_var)
        z_context = (z_con_mu, z_con_var)

        loss = np_loss(y_mean, sigma, y_all, z_all, z_context)
        loss.backward()
        train_loss += loss.item()
        optimizer.step()
        if batch_idx % args.log_interval == 0:
            print('Train Epoch: {} [{}/{} ({:.0f}%)]\tLoss: {:.6f}'.format(
                epoch, batch_idx * len(y_all), batch_nums * batch_idx,
                       100. * batch_idx / batch_nums,
                       loss.item() / len(y_all)))
        if batch_idx % 100 == 0:
            # log rec images
            ...
            # recons = y_hat[:num_examples].view(-1, channel_dim, channel_dim, c)

            context_pixels = y_all[:num_examples].view(-1, channel_dim, channel_dim, c).permute(0, 3, 1, 2)
            # context_pixels = y_all[:num_examples].view(-1, channel_dim, channel_dim, 3)
            background = context_pixels
            comparison = torch.cat([background,
                                    recons]) + 0.5
            # print(comparison.shape)
            # print(comparison)
            path = os.path.join(args.log_dir, 'train')
            if os.path.exists(path) == False:
                os.makedirs(path)
            save_image(comparison.cpu(),
                       path + '/%s_ep_' % (args.dataset + '_' + args.att_type) + str(epoch) + '.png',
                       nrow=num_examples)
            # assert False

    # print('====> Epoch: {} Average loss: {:.4f}'.format(
    #     epoch, train_loss / (data_num / float(args.batch_size))))
...
def test(epoch):
    ...
    test_loss = 0
    with torch.no_grad():
        for i, (y_all, _) in enumerate(val_loader):
            if i > 5000:
                break
            batch_size = y_all.shape[0]
            ...
            y_mean, sigma, z_all_mu, z_all_var, z_con_mu, z_con_var = model(x_context, y_context)
            z_all = (z_all_mu, z_all_var)
            z_context = (z_con_mu, z_con_var)
            test_loss += np_loss(y_mean, sigma, y_all, z_all, z_context).item()
            ...
            if i == 0:  # save PNG of reconstructed examples
                ...
                # print(comparison.shape)
                # print(comparison)
                path = os.path.join(args.log_dir, 'test')
                if os.path.exists(path) == False:
                    os.makedirs(path)
                save_image(comparison.cpu(),
                           path + '/%s_ep_' % (args.dataset + '_' + args.att_type) + str(epoch) + '.png',
...
if __name__ == '__main__':
    ...
    for epoch in range(1, args.epochs + 1):
        train(epoch)
        test(epoch)
        torch.save(model, os.path.join(args.restore_model, (args.att_type + '_model_%d.pkl') % (epoch)))
